...
def normalize (A):
    ...
    new_matrix=A/column_sum[np.newaxis,:]
    return new_matrix
...
def draw(G,A,cluster_map):
    ...
    plt.figure(2)
    nx.draw_network_nodes(G,pos,node_size=200,node_color=colors,cmap=plt.cm.Blues)
    nx.draw_network_edges(G,pos,alpha=0.5)
...
def network_mcl(G,expand_factor=2,inflate_factor=2,maxloop=10,mult_factor=1):
    import networkx as nx
